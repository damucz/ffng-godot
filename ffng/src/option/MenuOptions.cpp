...
MenuOptions::createSoundPanel(const Labels &labels)
{
    ...
    soundBox->addWidget(new FSlider("volume_sound", 0, 100));
...
MenuOptions::createMusicPanel(const Labels &labels)
{
    ...
    musicBox->addWidget(new FSlider("volume_music", 0, 100));
...
MenuOptions::createStatusBar(int width)
{
    ...
    FFont *new_font = new FFont(Path::dataReadPath("font/font_menu.ttf"), 16);
...
#else
    SDL_FillRect(screen, nullptr, gray);
#endif
}
//-----------------------------------------------------------------
/**
