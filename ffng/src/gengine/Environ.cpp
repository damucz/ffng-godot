...
#else
    Error error;
    FileAccess* config = FileAccess::open(file.getNative().c_str(), FileAccess::WRITE, &error);
    if (error == OK) {
        config->store_string("-- this file is automatically generated\n");

        t_values::iterator end = m_values.end();
        for (t_values::iterator i = m_values.begin(); i != end; ++i) {
            String s = String("setParam(\"") + i->first.c_str() + "\", \"" + i->second.c_str() + "\")\n";
