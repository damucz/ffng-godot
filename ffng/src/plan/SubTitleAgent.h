...
class SubTitleAgent : public BaseAgent, public Drawable {
    ...
    int m_limitY;
    private:
    std::string splitAndCreate(const std::string &subtitle, const FColor *color);
    void trimRest(std::string &buffer);
    void newShortSubtitle(const std::string &subtitle, const FColor *color);
    ...
    void addFont(const std::string &fontname, FColor *new_color);
